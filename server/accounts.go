...
type serviceImport struct {
	...
	implicit    bool        // generated by the server for system event requests
}
...
func (a *Account) addServiceImportWithClaim(destination *Account, from, to string, imClaim *jwt.Import, internal bool) error {
	...
	_, err := a.addServiceImport(destination, from, to, imClaim, internal && imClaim == nil)
...
func (a *Account) hasServiceExportExact(subject string) bool {
	a.mu.RLock()
	_, ok := a.exports.services[subject]
	a.mu.RUnlock()
	return ok
}
...
// hasImplicitServiceImportForSubject reports whether the exact subject has an
// implicit service import to the given account. An exact implicit import takes
// precedence over a broader user import to that account.
func (a *Account) hasImplicitServiceImportForSubject(dst *Account, subject string) bool {
	a.mu.RLock()
	defer a.mu.RUnlock()
	for _, si := range a.imports.services[subject] {
		if si.implicit && si.acc == dst {
			return true
		}
	}
	return false
...
func (a *Account) addServiceImport(dest *Account, from, to string, claim *jwt.Import, implicit bool) (*serviceImport, error) {
	rt := Singleton
	...
	si := &serviceImport{dest, claim, se, nil, from, to, tr, 0, rt, lat, nil, nil, nil, usePub, false, false, share, false, false, atrc, nil, implicit}
...
func (a *Account) addRespServiceImport(dest *Account, to string, osi *serviceImport, tracking bool, header http.Header, mt *msgTrace) *serviceImport {
	...
	si := &serviceImport{dest, nil, osi.se, nil, nrr, to, nil, 0, rt, nil, nil, nil, mt, false, true, false, osi.share, false, false, false, nil, false}
...
func (a *Account) getWildcardServiceExport(from string) *serviceExport {
	...
	var best *serviceExport
	bestLiterals, bestWildcards, bestTokens := -1, -1, -1
	var bestSubject string
	for subj, se := range a.exports.services {
		if !isSubsetMatch(tokens, subj) {
			continue
		}
		// Exports can overlap. Prefer the filter with more literal tokens, then
		// more single-token wildcards, so map iteration order cannot change the
		// response type selected for an import.
		parts := strings.Split(subj, tsep)
		...
		moreSpecific := literals > bestLiterals ||
			(literals == bestLiterals && wildcards > bestWildcards) ||
			(literals == bestLiterals && wildcards == bestWildcards && len(parts) > bestTokens) ||
			(literals == bestLiterals && wildcards == bestWildcards && len(parts) == bestTokens &&
				(bestSubject == _EMPTY_ || subj < bestSubject))
		if moreSpecific {
			best, bestSubject = se, subj
			bestLiterals, bestWildcards, bestTokens = literals, wildcards, len(parts)
		}
	}
	return best
}
...
func (s *Server) updateAccountClaimsWithRefresh(a *Account, ac *jwt.AccountClaims, refreshImportingAccounts bool) {
	...
	if a.imports.services != nil {
		...
		for k, v := range a.imports.services {
			...
			for _, si := range v {
				if si.implicit && a != s.SystemAccount() {
					// These imports are server-managed rather than claim-managed.
					// Keep their subscriptions active across the claim rebuild.
					a.imports.services[k] = append(a.imports.services[k], si)
					continue
				}
				old.imports.services[k] = append(old.imports.services[k], si)
			}
		}
	}
