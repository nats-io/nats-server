				...
				for _, bc := range benchmarksCases {
					...
					b.Run(
						...
						func(b *testing.B) {
							...
							// - Unsubscribe
							subscribeConsumeUnsubscribe := func(js nats.JetStreamContext) {
							...
							for i := 1; i <= bc.concurrentConsumers; i++ {
								go func(consumerId int) {
									...
									for opsCount.Add(-1) >= 0 {
										subscribeConsumeUnsubscribe(js)
										completed += 1
									}
									if verbose {
...
func BenchmarkJetStreamObjStore(b *testing.B) {
	...
	RunObjStoreBenchmark := func(b *testing.B, objStore nats.ObjectStore, minObjSz int, maxObjSz int, numKeys int, rwRatio float64) (int, int, int) {
		...
		rand.NewChaCha8([32]byte{seed}).Read(dataBuf)
		...
		for i := 1; i <= b.N; i++ {
			key := fmt.Sprintf("%s_%d", keyPrefix, rand.IntN(numKeys))
			var err error
			...
			case rwOp > rwRatio:
				...
				dataSz := rand.IntN(maxObjSz-minObjSz+1) + minObjSz
				data := dataBuf[:dataSz]
