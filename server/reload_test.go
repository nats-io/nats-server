...
func TestConfigReloadGetLeafNodeRemoteName(t *testing.T) {
	urlStr := "nats://127.0.0.1:7222"
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			name := getLeafNodeRemoteName(test.input)
...
func TestConfigReloadGetLeafNodeOptionsChanges(t *testing.T) {
	...
	for _, test := range []struct {
		...
		},
		{
			...
			nil,
			fmt.Sprintf("remote %s: field %q was false now true", getLeafNodeRemoteName(remote), "Hub"),
		},
		{
...
func TestConfigReloadLeafNodeUnsupportedChangesFail(t *testing.T) {
	...
	`
	conf2 := createConfFile(t, []byte(fmt.Sprintf(tmpl2, -1, o1.LeafNode.Port, _EMPTY_)))
	...
	remoteName := getLeafNodeRemoteName(remote)
	require_NotEqual(t, remoteName, _EMPTY_)

	// Make a change that is not supported in the main block
	err := os.WriteFile(conf2, fmt.Appendf(nil, tmpl2, 1234, o1.LeafNode.Port, _EMPTY_), 0666)
	require_NoError(t, err)
	err = s2.Reload()
	require_Error(t, err)
	require_Contains(t, err.Error(), "not supported for LeafNode: field \"Port\" was")

	// Now make one in the remote block
	err = os.WriteFile(conf2, fmt.Appendf(nil, tmpl2, -1, o1.LeafNode.Port, ", hub: true"), 0666)
	require_NoError(t, err)
	err = s2.Reload()
	require_Error(t, err)
	require_Contains(t, err.Error(),
		fmt.Sprintf("not supported for LeafNode: remote %s: field %q was false now true",
			remoteName, "Hub"))
}
...
func TestConfigReloadAddRemoveRemoteLeafNodes(t *testing.T) {
	...
	conf2 := createConfFile(t, []byte(fmt.Sprintf(tmpl2, accA, _EMPTY_)))
	s2, _ := RunServerWithConfig(conf2)
	defer s2.Shutdown()
	...
	checkLeafNodeConnectedCount(t, s2, 2)
	checkLeafs([]string{"A", "B"})

	// Remote remote with local account "A"
