...
func newFileStoreWithCreated(fcfg FileStoreConfig, cfg StreamConfig, created time.Time, prf, oldprf keyGen) (*fileStore, error) {
	...
	if dios == nil {
		// When running in unit tests as an isolated store.
		dios = make(chan struct{}, numIOs())
		for i := 0; i < cap(dios); i++ {
			dios <- struct{}{}
		}
	}
...
func (fs *fileStore) writeFileWithOptionalSync(name string, data []byte, perm fs.FileMode) error {
	if fs.fcfg.SyncAlways {
		return writeFileWithSync(name, data, perm, fs.dios)
	}
	<-fs.dios
	defer func() {
		fs.dios <- struct{}{}
...
func writeFileWithSync(name string, data []byte, perm fs.FileMode, dios chan struct{}) error {
	<-dios
	defer func() {
		dios <- struct{}{}
	}()
