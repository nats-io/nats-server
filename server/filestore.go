...
func (mb *msgBlock) setupWriteCache(buf []byte) {
	...
	// Check if we have a weak reference that we can reuse.
	if mbcache := mb.ecache.Value(); mbcache != nil {
		mb.cache = mbcache
		return
	}
...
func (mb *msgBlock) clearCache() {
	...
	// It's possible that mb.cache will be nil here, so we're
	// effectively clearing out the elastic pointer in that case.
	mb.ecache.Weaken()
	mb.ecache.Set(mb.cache)
...
checkCache:
	...
	// Before we check if the cache is already loaded, we will need to
	// strengthen our elastic reference if possible. This might still
	// end up being nil but that's OK, we'll catch that next.
	if mb.cache == nil {
