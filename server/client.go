...
func (c *client) processInboundClientMsg(msg []byte) {
	...
	if hasGWRoutedPrefix(c.pa.subject) {
		c.sendErrAndErr(ErrReservedPublishSubject.Error())
		return
	}
	...
	c.mu.Lock()
	if len(c.gwrm) > 0 {
		if rm, ok := c.gwrm[string(c.pa.subject)]; ok {
...
func (c *client) processMsgResults(acc *Account, r *SublistResult, msg, subject, reply []byte, flags int) [][]byte {
	...
	var removeGWPrefix bool
	var creply = reply
	if len(reply) > gwSubjectOffset && string(reply[:gwReplyPrefixLen]) == gwReplyPrefix {
		removeGWPrefix = true
	...
	for _, sub := range r.psubs {
		...
		// Normal delivery
		if removeGWPrefix {
			c.srv.trackGWReply(sub.client, string(creply), string(reply))
		}
	...
	selectQSub:
		...
		for startIndex, i := c.in.prand.Intn(len(qsubs)), 0; i < len(qsubs); i++ {
			...
			rreply := reply
			if removeGWPrefix && sub.client.kind == CLIENT {
				c.srv.trackGWReply(sub.client, string(creply), string(reply))
				rreply = creply
			}
