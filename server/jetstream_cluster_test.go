...
func TestJetStreamClusterRedeliverBackoffs(t *testing.T) {
	...
	// Test when BackOff is configured and AckWait and MaxDeliver are as well.
	// Currently the BackOff will override any other settings.
	// TODO(dlc) - We could error as well too?
	ccReq := &CreateConsumerRequest{
		Stream: "TEST",
		Config: ConsumerConfig{
			Durable:        "dlc",
			...
			AckWait:        30 * time.Second,
			MaxDeliver:     10,
			BackOff:        []time.Duration{25 * time.Millisecond, 250 * time.Millisecond, 500 * time.Millisecond},
		},
	}
	req, err := json.Marshal(ccReq)
	require_NoError(t, err)
	...
	if cfg := ccResp.ConsumerInfo.Config; cfg.AckWait != 25*time.Millisecond || cfg.MaxDeliver != 4 {
		t.Fatalf("Expected AckWait to be first BackOff (25ms) and MaxDeliver set to len(BackOff)+1, got %+v", cfg)
	}
	...
	checkFor(t, 5*time.Second, 500*time.Millisecond, func() error {
		...
		return fmt.Errorf("Only seen %d of 4", nr)
	})
	sub.Unsubscribe()
