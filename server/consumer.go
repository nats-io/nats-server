...
type consumer struct {
	...
	rsm       map[string]struct{} // Reset requests that need to be responded to on the internal sys account.
	prOk      bool
	uch       chan struct{}
...
VALID:
	...
	if o.node != nil {
		...
		if internal {
			if o.rsm == nil {
				o.rsm = make(map[string]struct{}, 1)
			}
			o.rsm[reply] = struct{}{}
		}
	...
	waitForMsgs:
		...
		case <-hbc:
			if o.isActive() {
				...
				flowControl := o.cfg.AckPolicy == AckFlowControl && len(o.pending) > 0 && o.fcid == _EMPTY_
				o.mu.RUnlock()
