...
type route struct {
	...
	// Subscriptions received over this route for accounts not yet known.
	// Installed when a config reload introduces the account.
	pendingSubs map[string]*pendingRouteSub
}

// pendingRouteSub stores interest received over a route for an unknown account
// so it can be installed if the account is subsequently added during a reload.
...
// maxPendingRouteSubsPerRoute caps the held interest for unconfigured
...
func (c *client) processRemoteUnsub(arg []byte, leafUnsub bool) (err error) {
	...
	if v, ok := srv.accounts.Load(accountName); ok {
		...
		c.mu.Lock()
		canceled := c.cancelPendingRemoteSub(key)
		c.mu.Unlock()
		if !canceled {
...
func (c *client) processRemoteSub(argo []byte, leafSub, hasOrigin bool) (err error) {
	...
	} else if c.route != nil && c.route.pendingSubs != nil {
		delete(c.route.pendingSubs, key)
	}
	c.mu.Unlock()
...
func (c *client) holdRemoteSubForUnknownAccount(accountName string, sub *subscription) {
	...
	// Recheck if the account was registered concurrently during a config reload
	// while holding the route connection lock to eliminate reload-vs-RS+ races.
	srv := c.srv
	if srv != nil {
		var acc *Account
		if v, ok := srv.accounts.Load(accountName); ok {
...
// Returns true when held interest was canceled.
