...
func TestConsumerRedeliveryQueueInterleaved(t *testing.T) {
	...
	for i := 0; i < 10000; i++ {
		...
		require_Equal(t, o.rdq.Len(), len(expected))
		var actual []uint64
		o.rdq.Range(func(seq uint64) bool {
			actual = append(actual, seq)
			return true
		})
		require_True(t, slices.Equal(actual, expected))
