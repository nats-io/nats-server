...
func require_True(t *testing.T, b bool) {
	t.Helper()
	if !b {
		assert.Always(false, t.Name()+" - Required true is false", nil)
...
func require_False(t *testing.T, b bool) {
	t.Helper()
	if b {
		assert.Always(false, t.Name()+" - Required false is true", nil)
...
func require_Equal[T comparable](t *testing.T, a, b T) {
	t.Helper()
	if a != b {
		assert.Always(false, t.Name()+" - Required equal is not equal", nil)
