...
func (n *raft) processAppendEntry(ae *appendEntry, sub *subscription) {
	...
	//if len(ae.entries) > 0 {
	n.debug("DEBUG: processAppendEntry ae.pindex=%d, ae.pterm=%d (term=%d, lterm=%d), ae.commit=%d, len(ae.entries)=%d, leader=%s", ae.pindex, ae.pterm, ae.term, ae.lterm, ae.commit, len(ae.entries), ae.leader)
	//}
