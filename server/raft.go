...
func (n *raft) maxBatchSize() int {
	...
	if acc != nil {
		acc.mu.RLock()
		if acc.mpay > 0 {
			maxPayload = min(maxPayload, int(acc.mpay))
		}
		acc.mu.RUnlock()
	}
...
func (n *raft) runAsLeader() {
	...
	for n.State() == Leader {
		...
		case <-n.prop.ch:
			...
			const batchBaseLen = appendEntryBaseLen + binary.MaxVarintLen64
			var entries []*Entry

			es, sz := n.prop.pop(), batchBaseLen
			for _, b := range es {
				if b.term != term {
					continue
				}
