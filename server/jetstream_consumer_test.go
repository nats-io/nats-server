...
func TestJetStreamConsumerResetToSequence(t *testing.T) {
	test := func(replicas int) {
		...
		require_Error(t, resp.Error, NewJSInvalidJSONError(errors.New("reset to zero seq")))
...
func TestJetStreamConsumerAckFlowControlBasics(t *testing.T) {
	test := func(replicas int) {
		...
		// Only the deliver subject, policy and heartbeat are required; others are automatically defaulted.
		cfg := ConsumerConfig{
			Durable:        "DEFAULT",
		...
		cfg.MaxDeliver = 0
		_, err = jsConsumerCreate(t, nc, "TEST", cfg, true)
		require_Error(t, err, NewJSConsumerWithFlowControlNeedsHeartbeatsError())

		cfg.Heartbeat = time.Second
		ccfg, err = jsConsumerCreate(t, nc, "TEST", cfg, true)
		require_NoError(t, err)
		checkConfig(ccfg)
	}
