...
func TestJetStreamConsumerUnpin(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{"unpin non-existing group, clustered", cnc, "TEST", "C", "B", &ApiError{ErrCode: uint16(JSConsumerInvalidPriorityGroupErr)}},
		{"unpin on missing stream, clustered", cnc, "NOT_EXIST", "C", "A", &ApiError{ErrCode: uint16(JSStreamNotFoundErr)}},
		{"unpin on missing consumer, clustered", cnc, "TEST", "NOT_EXIST", "A", &ApiError{ErrCode: uint16(JSConsumerNotFoundErr)}},
		{"ok unpin, clustered", cnc, "TEST", "C", "A", nil},
	} {
		t.Run(test.name, func(t *testing.T) {
			err := unpinRequest(t, test.nc, test.stream, test.consumer, test.group)
			if test.err != nil {
