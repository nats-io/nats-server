...
func TestJetStreamClusterKeyValueSync(t *testing.T) {
	...
	checkState := func(t *testing.T, c *cluster, accountName, streamName string) error {
		...
		for _, srv := range c.servers {
			...
			stream := getStreamDetails(t, c, accountName, streamName)
			if stream == nil {
				return fmt.Errorf("stream not found")
			}
	...
	checkMsgsEqual := func(t *testing.T, accountName, streamName string) error {
		...
		// 		a.mu.RLock()
		// 		b.mu.RLock()
		// 		state1 := a.store.State()
		// 		state2 := b.store.State()
		// 		if state1.FirstSeq != state2.FirstSeq {
		// 			t.Logf("Replicas FirstSeq is different: %v vs %v", state1.FirstSeq, state2.FirstSeq)
		// 		}
		// 		if state1.LastSeq != state2.LastSeq {
		// 			t.Logf("Replicas LastSeq is different: %v vs %v", state1.LastSeq, state2.LastSeq)
		...
		// leaderSrv := c.streamLeader(accountName, streamName)
		// if leaderSrv == nil {
		// 	return fmt.Errorf("could not find stream leader")
		// }
			...
			for i := 0; i < 5; i++ {
				...
				if err != nil {
					...
					if k != nil && abs(int64(k.Revision())-int64(nextk.Revision())) > 2 {
						t.Logf("get-revision-error:[%s/%s] [%d] [%d]", kvname, key, k.Revision(), nextk.Revision())
					}
					k = nextk
				}
			}
...
Loop:
	for range time.NewTicker(20 * time.Second).C {
		...
		for _, str := range streams {
			checkState(t, c, "js", str)
			checkMsgsEqual(t, "js", str)
		}
	}

	wg.Wait()

	// Do another check at the end.
	for _, str := range streams {
		checkState(t, c, "js", str)
		checkMsgsEqual(t, "js", str)
	}
