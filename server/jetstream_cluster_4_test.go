...
func TestJetStreamClusterSourcingConsumerMoveDoesNotReset(t *testing.T) {
	...
	// A re-proposal must not answer the stale create reply of the sourcing
	// consumer, which would reset it.
	snc, _ := jsClientConnect(t, c.randomServer(), nats.UserInfo("admin", "s3cr3t!"))
	defer snc.Close()
	replies := make(chan *nats.Msg, 16)
	sub, err := snc.ChanSubscribe("_R_.>", replies)
	require_NoError(t, err)
	...
	require_NoError(t, err)
	checkFor(t, 10*time.Second, 200*time.Millisecond, func() error {
		sl := c.streamLeader(globalAccountName, "ORIGIN")
		if sl == nil {
			return errors.New("no stream leader yet")
		}
		if sl.Name() != "S-2" {
			return fmt.Errorf("expected stream leader S-2, got %s", sl.Name())
		}
		return nil
	})
	...
	for {
		...
		case m := <-replies:
			...
			require_NotEqual(t, resp.Type, JSApiConsumerResetResponseType)
			continue
		default:
		}
		break
