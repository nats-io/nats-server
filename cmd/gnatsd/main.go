...
func main() {
	...
	if configFile != "" {
		fileOpts, err := gnatsd.ProcessConfigFile(configFile)
		if err != nil {
			gnatsd.PrintAndDie(err.Error())
		}
		opts = *gnatsd.MergeOptions(fileOpts, &opts)
	}

	// Remove any host/ip that points to itself in Route
	newroutes, err := gnatsd.RemoveSelfReference(opts.ClusterPort, opts.Routes)
	if err != nil {
		gnatsd.PrintAndDie(err.Error())
	}
...
func configureAuth(s *gnatsd.Server, opts *gnatsd.Options) {
	if opts.Username != "" {
		auth := &auth.Plain{
...
func configureLogger(s *gnatsd.Server, opts *gnatsd.Options) {
	var log gnatsd.Logger
